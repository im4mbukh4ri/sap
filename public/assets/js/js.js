...
$(function() {
  ...
  $("#train_departure_date").datepicker({
		...
		var minDate = new Date(selected.date.valueOf());
		$('#train_return_date').datepicker('setStartDate', minDate);
	});
	$("#train_return_date").datepicker({
		autoclose:true
	});
		// $( "#departure_date" ).datepicker({ minDate: -0 }).on("changeDate", function (e) {
		// 	var minDate = new Date(e.date.valueOf());
		// 	$( "#return_date" ).datepicker('setEndDate',minDate);
		// });
		// $( "#train_departure_date" ).datepicker({ minDate: -0, maxDate: "+3M" });
		...
		$( "#train_adult_count" ).spinner({
			min: 1
		});
		$( "#train_child_count" ).spinner( {
			min: 0
		});
