...
class PlnPascabayarReplication extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cron:pln_pascabayar_replication';
    ...
    protected $description = 'Replikasi pln pascabayar';
    ...
    {
        $transactions = PpobTransaction::where('service', '=', 3)
        ...
        foreach ($transactions as $pulsa) {
            ...
            try {
                ...
                $detail = new PostpaidElectricity;
                $detail->customer_name = $pulsa->pln_pasca->customer_name;
                $detail->created_at = $pulsa->created_at;
                $detail->tariff_adjustment = $pulsa->pln_pasca->golongan_daya;
                $detail->bill_amount = $pulsa->pln_pasca->nominal;
                $detail->admin_amount = $pulsa->pln_pasca->admin;
                $detail->stand_meter = $pulsa->pln_pasca->stand_meter;
                $detail->period = $pulsa->pln_pasca->period;
                $detail->supplier_service_code = 'PLN';
                $detail->supplier_product_code = 'PAS';
                ...
                $postpaid->admin_amount = $pulsa->pln_pasca->admin;
                ...
                $postpaid->detailable_type = 'App\Models\Postpaid\PostpaidElectricity';
