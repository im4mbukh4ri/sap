...
class RequestTransfer
{
    ...
    public function requestTransfer()
    {
        ...
        $message = 'Silahkan masukan kode berikut untuk melanjutkan proses transfer deposit : ';
        if ($checkRequest) {
                ...
                if ($expired <= 0 && $value->to_user == $this->toUser) {
                    return $this->response = [
                        'status' => [
                            'code' => 400,
                            'confirm' => 'failed',
                            'message' => 'Proses gagal. Anda baru saja melakukan request transfer deposit sebelumnya ke member yang sama. Silahkan ulangi kembali setelah 5 menit.',
                        ],
                    ];
                }
                $value->status = 3;
