...
use App\TrainScheduleFare;
use App\TrainTransaction;
use Illuminate\Http\Request;
use Response;
use Validator;

class ApiTrainsController extends Controller {
	//
	private $rqid;
	private $mmid;
	private $app;
...
//		}
	...
	public function getSchedule(Request $request) {
		...
		$schedule = SipTrain::GetSchedule(json_encode($param))->get();
		if ($schedule['error_code'] == "000") {
			foreach ($schedule['schedule']['departure'] as $key => $value) {
				$saveSchedule = TrainSchedule::firstOrCreate([
				...
				foreach ($value['Fares'] as $val => $fare) {
					$find = TrainScheduleFare::find($fare['selectedIDdep']);
					if (!$find) {
						$saveSchedule->fares()->save(
							new TrainScheduleFare([
                    ...
                    $schedule['schedule']['departure'][$key]['Fares'][$val]['market_price'] = $fare['TotalFare'];
				}
			}
			if ($request->trip == "R") {
				foreach ($schedule['schedule']['return'] as $key => $value) {
					...
					foreach ($value['Fares'] as $val => $fare) {
						$find = TrainScheduleFare::find($fare['selectedIDret']);
						if (!$find) {
							$saveSchedule->fares()->save(
								new \App\TrainScheduleFare([
                        ...
                        $schedule['schedule']['return'][$key]['Fares'][$val]['market_price'] = $fare['TotalFare'];
					}
				}
			}
			$response = [
	...
	public function getSeat(Request $request) {
		...
		$seat = SipTrain::GetSeat(json_encode($param))->get();
	...
	public function bookingIssued(Request $request) {
		...
		$user = $client->user;
		$selectedDep = TrainScheduleFare::find($request->selectedIDdep);
...
//                $param['departure']['seatinf_'.$i]=$seat[0];
		...
		$param['admin'] = 7500;
		$param['adminDep'] = $param['admin'];
		if ($request->trip == "R") {
			$param['adminDep'] = 7500;
			$param['adminRet'] = 7500;
			$param['admin'] = $param['adminDep'] * 2;
			$param['indexFareRet'] = 0;
			$selectedRet = TrainScheduleFare::find($request->selectedIDret);
			$param['selectedIDret'] = $request->selectedIDret;
			$param['tgl_ret'] = $request->tgl_ret;
...
//                $seat=explode(',',$request->get('seatinf_'.$i));
		...
		// return $param;
		$result = SipTrain::CreateTransaction($user->id, $param)->get();
	...
	public function reports(Request $request) {
		...
		if ($request->has('start_date') && $request->has('end_date')) {
			...
			if (daysDifference($until, $from) > 31) {
				$this->setStatusMessage('Report kereta api yang bisa Anda cek maksimal 31 hari.');
				return Response::json([
					'status' => [
						'code' => $this->getCodeFailed(),
						'confirm' => $this->getConfirmFailed(),
