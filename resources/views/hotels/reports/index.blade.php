...
@section('content')
		 <section class="section-pilih-seat">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
					 	<h3 class="blue-title-big">HISTORY HOTEL</h3>
					 	<div id="dataSearch" class="box-orange">
							<form action="{{route('hotels.reports')}}" method="GET">
								{{csrf_field()}}
							 <div class="row-form">
								 <div class="row">
									<div class="col-md-3">
    									...
    									@forelse($transactions as $key => $transaction)
												<tr>
													...
													{{-- <td style="text-align: right;">
														Smart Point : {{ (isset($transaction->commission))?number_format($transaction->commission->bv):'' }}<br>
														Smart Cash : {{ (isset($transaction->commission))?number_format($transaction->commission->member):'' }}
													</td> --}}
													@if(isset($transaction->commission))
													<?php $smartPrice+=$transaction->total_fare-$transaction->commission->member; ?>
														<td style="text-align: right;">{{number_format($transaction->total_fare-$transaction->commission->member)}}</td>
													@else
													<?php $smartPrice+=$transaction->total_fare; ?>
														<td style="text-align: right;">{{number_format($transaction->total_fare)}}</td>
													@endif
													<td>
                            <?php
                          ...
                          <td>
													...
													<td>
														@if($transaction->status=='issued')
															<form action="{{ route('hotels.receipt',$transaction->id) }}" method="GET" target="_blank"><input type="hidden" name="service_fee" class="service_fee" value="0">
								...
								<div class="col-md-4">
										<table class="text-right dataTable table table-striped table-bordered dt-responsive nowrap table-custom-blue">
											...
											<tr>
													...
													<td>IDR {{ number_format($smartPrice) }}</td>
											</tr>
											{{-- <tr>
