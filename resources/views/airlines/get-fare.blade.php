...
?>
                                    @if((int)$result['NTA']>0)
                                        <?php
$totalNRA = (int) $result['TotalAmount'] - (int) $result['NTA'];
$komisi90 = (int) ($totalNRA * $percentCommission) / 100;
$komisiMember = (int) ($komisi90 * $percentSmartCash) / 100;
$estimatePrice = (int) $result['TotalAmount'] - $komisiMember;
?>
                                        <h3 id="summary_pricetotal">IDR {{number_format($estimatePrice)}} <sup style="text-decoration: line-through;color: red;">IDR {{number_format($result['TotalAmount'])}}</sup></h3>
                                    @else
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-8">
                    ...
                    {{--{!! Form::open(['route'=>'airlines.booking','method'=>'post','id'=>'finishForm']) !!}--}}
                    {!! Form::hidden('acDep',getAC($result['schedule']['departure'][0]['Flights'][0]['FlightNo'])) !!}
                    {!! Form::hidden('org',$result['org']) !!}
                    {!! Form::hidden('des',$result['des']) !!}
                    {!! Form::hidden('tgl_dep',$result['tgl_dep']) !!}
                    {!! Form::hidden('selectedIDdep',getSelectedIdDepFare($result['schedule']['departure'][0]['Fares'])) !!}
                    {!! Form::hidden('flight',$result['flight']) !!}
                    {!! Form::hidden('adt',$result['adt']) !!}
                    {!! Form::hidden('chd',$result['chd']) !!}
                    {!! Form::hidden('inf',$result['inf']) !!}
                    {!! Form::hidden('estimatePrice',$estimatePrice) !!}
                    {!! Form::hidden('username',$username) !!}
                    {!! Form::hidden('bookStat',$result['bookStat']) !!}
                    {!! Form::hidden('totalFare',$result['TotalAmount']) !!}
                    <input type='hidden' name='result' value='{{json_encode($result)}}'>
                    @if($result['flight']=="R")
                        {!! Form::hidden('acRet',getAC($result['schedule']['return'][0]['Flights'][0]['FlightNo']))!!}
                        {!! Form::hidden('tgl_ret',$result['tgl_ret']) !!}
                        {!! Form::hidden('selectedIDret',getSelectedIdRetFare($result['schedule']['return'][0]['Fares'])) !!}
                    @endif
                    ...
                    @for($i=1; $i<=(int)$result['adt'];$i++)
                        <div class="left-section">
                            ...
                            <div class="grey-box">
                                ...
                                @if($result['flight']=="O")
                                    @if(getAC($result['schedule']['departure'][0]['Flights'][0]['FlightNo'])=='QZ'||$std->domestic != 1||$sta->domestic != 1 || $std->id == 'DIL' || $sta->id == 'DIL')
                                        <hr>
                                    <h3>Data Passpor</h3>
                                        <div class="row">
                                    ...
                                    @endif
                                @else
                                    @if(getAC($result['schedule']['departure'][0]['Flights'][0]['FlightNo'])=='QZ'||getAC($result['schedule']['return'][0]['Flights'][0]['FlightNo'])=='QZ'||$std->domestic != 1||$sta->domestic != 1)
                                        <hr>
                                        <h3>Data Passpor</h3>
                                        <div class="row">
                                            <div class="col-md-3 col-xs-12">
                        ...
                        @for($i=1; $i<=(int)$result['chd'];$i++)
                            <div class="left-section">
                                ...
                                <div class="grey-box">
                                    ...
                                    @if($result['flight']=="O")
                                        @if(getAC($result['schedule']['departure'][0]['Flights'][0]['FlightNo'])=='QZ'||$std->international==1||$sta->international==1)
                                            <hr>
                                            <h3>Data Passpor</h3>
                                        ...
                                        @endif
                                    @else
                                        @if(getAC($result['schedule']['departure'][0]['Flights'][0]['FlightNo'])=='QZ'||getAC($result['schedule']['return'][0]['Flights'][0]['FlightNo'])=='QZ'||$std->international==1||$sta->international==1)
                                            <hr>
                                            <h3>Data Passpor</h3>
                                            <p>Jika anak belum memiliki paspor, dapat menggunakan data paspor orang tua ( penumpang dewasa ).</p>
                                            <div class="row">
                    ...
                    <div class="left-section">
                            ...
                            <div class="rows text-right" style="padding: 0;">
                                <div class="col-md-4 col-md-offset-8">
                                    ...
                                    @else
                                        <input type="submit" class="btn btn-cari" value="SELANJUTNYA">
                                    @endif
                                </div>
                            </div>
                        {!! Form::close() !!}
                            ...
                            <div class="right-info">
                                ...
                                @foreach($airlines as $key => $airlineList)
                                    <div class="white-box small-flight">
                                        <div class="items-flight-destination">
                                            <div class="row-detail">
                                                <img width="80" src="{{asset('/assets/logo/'.getLogo($airlineList['FlightNo']).'.png')}}" alt="logo-{{$airline[$result['acRet']]['name']}}"><br>
                                                <span class="code-penerbangan">{{$airlineList['FlightNo']}}</span>
                                            </div><!--end.row-detail-->
                                            <div class="row-detail">
                                                <div class="timeline-flight">
