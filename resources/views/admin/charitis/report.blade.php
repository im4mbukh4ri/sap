...
@section('content')
		 <section class="section-pilih-seat">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
					 	<h3 class="blue-title-big">DATA CHARITY</h3>
					 	<div id="dataSearch" class="box-orange">
							<form action="{{route('admin.transactions_charity')}}" method="GET">
								{{csrf_field()}}
							 <div class="row-form">
								 <div class="row">
									<div class="col-md-3">
									...
									<div class="col-md-3">
                                        <div class="form-group">
                                            <label class="whiteFont">Program</label>
                                            <div class="input-group custom-input full-width">
                                           <?php $idcharity = App\Charity::get();?>
                                                <select class="form-control full-width" name="charity_id">
                                                    <option value="">ALL</option>
                                                    @foreach($idcharity as $key => $charityid)
                                                        @if(old('charity_id')==$charityid->id)
                                                            <option value="{{ $charityid->id }}" selected>{{$charityid->title}}</option>
                                                        @else
                                                            <option value="{{ $charityid->id }}">{{$charityid->title}}</option>
                                                        @endif
                                                    @endforeach
                                                </select>
                                            </div>
...
?>
														@endif</td>
													<td><a href="#" class="btn btn-md btn-primary"><span class="fa fa-search"></span></a></td>
												</tr>
												@endforeach
