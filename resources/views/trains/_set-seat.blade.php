...
@section('content')
  <?php
    ...
    $std=getStation($request['org']);
    $sta=getStation($request['des']);
                            ...
                            <div class="summary-flight">
                                ...
                                <div class="table-responsive">
                                    <table class="table result-tab">
                                        ...
                                        @foreach($request['infName'] as $key => $adt)
                                            <tr>
                                                <td>{{$request['infName'][$key]}}</td>
                                            </tr>
                                        @endforeach
                                    </table>
                                </div>
                                ...
                                <div v-if="success" >
                                    <div class="rows">
                                        <div class="alert-custom alert alert-green alert-dismissible" role="alert">
                                            ...
                                            <p><strong>Issued Berhasil!</strong><br>Silahkan klik data transaksi di bawah ini. <br><a href="{{route('trains.reports')}}" class="btn btn-cari">DATA TRANSAKSI</a></p>
                                        </div><!--end.alert-->
                                    </div>
                                </div>
                              </div>
                    ...
                    <div class="white-box no-padding-box">
                        <div class="detail-side-row rows">
                            <div class="text-center detail-top">
                                <div class="logo-kereta"><img src="{{asset('/assets/logo/kai.png')}}"></div>
                            ...
                            <div class="detail-bottom">
                                <dl class="dl-horizontal">
                                  ...
                                  @if($result['trip']=="O")
                                    <dt class="feature">{{$result['schedule']['departure'][$indexTrain]['TrainName']}}</dt><dd class="value">IDR {{number_format($result['schedule']['departure'][$indexTrain]['Fares'][$indexFare]['TotalFare'])}}</dd>
                                  @else
                                    <dt class="feature">{{$result['schedule']['departure'][$indexTrain]['TrainName']}} (Pergi)</dt><dd class="value">IDR {{number_format($result['schedule']['departure'][$indexTrain]['Fares'][$indexFare]['TotalFare'])}}</dd>
                                    <dt class="feature">{{$result['schedule']['return'][$indexTrainRet]['TrainName']}} (Pulang)</dt><dd class="value">IDR {{number_format((int)$result['schedule']['return'][$indexTrainRet]['Fares'][$indexFareRet]['TotalFare'])}}</dd>
                                  @endif
                                  <dt class="feature">Biaya Admin</dt><dd class="value">IDR {{number_format($request['admin'])}}</dd>
                        ...
                        <div class="total-side-rows">
                            <div class="row-orange">
                                <dl class="dl-horizontal">
                                  @if($result['trip']=="O")
                                    <dt class="feature">TOTAL</dt><dd class="value">IDR {{number_format($result['schedule']['departure'][$indexTrain]['Fares'][$indexFare]['TotalFare']+(int)$request['admin'])}}</dd>
                                  @else
                                    <dt class="feature">TOTAL</dt><dd class="value">IDR {{number_format((int)$result['schedule']['departure'][$indexTrain]['Fares'][$indexFare]['TotalFare']+(int)$result['schedule']['return'][$indexTrainRet]['Fares'][$indexFareRet]['TotalFare']+(int)$request['admin'])}}</dd>
                                  @endif
                                </dl>
                            </div>
                        </div>
            ...
            if(selectedSeat.length<pass){
              ...
              selectedSeat.forEach(function(item,index){
                if(index+1<=adt){
                  ...
                  console.log("inf seat = "+item);
                  $("#infSeat"+(index-adt)).val(item);
                  window.request.infSeat[index-adt]=item;
                }
              });
            }
              ...
              if(selectedSeatRet.length<pass){
                ...
                selectedSeatRet.forEach(function(item,index){
                  if(index+1<=adt){
                    ...
                    console.log("inf return seat = "+item);
                    $("#infSeatRet"+(index-adt)).val(item);
                    window.request.infSeatRet[index-adt]=item;
                  }
                });
              }
            }else if(res[4]==="selected"){
      ...
      methods:{
        trainTransaction:function(){
          ...
          if(this.term===true){
            ...
            // console.log("window request inf = "+parseInt(window.request.inf));
            if(window.request.adtSeat.length!=parseInt(window.request.adt)){
              this.seatNotSet=true;
              return
            }
//            if(parseInt(window.request.inf)!=0){
//              if(window.request.infSeat.length!=parseInt(window.request.inf)){
//                this.seatNotSet=true;
//                return
//              }
//            }
...
//              if(parseInt(window.request.inf)!=0){
//                if(window.request.infSeatRet.length!=parseInt(window.request.inf)){
//                  this.seatNotSet=true;
//                  return
//                }
//              }
