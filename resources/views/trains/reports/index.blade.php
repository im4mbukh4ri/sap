...
@section('content')
		 <section class="section-pilih-seat">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
					 	<h3 class="blue-title-big">HISTORY KERETA API</h3>
					 	<div id="dataSearch" class="box-orange">
							<form action="{{route('trains.reports')}}" method="GET">
								{{csrf_field()}}
							 <div class="row-form">
								 <div class="row">
									<div class="col-md-3">
    							...
    							<table id="data-table" class="text-center table table-striped table-bordered dt-responsive nowrap table-custom-blue">
    								...
    								<tbody>
    									@forelse($transactions as $key => $transaction)
											@foreach($transaction->bookings()->where('status', 'LIKE', '%' . $reqStatus . '%')->get() as $booking)
												<tr>
													<td><strong>{{$transaction->id}}</strong><br>{{date("d M y H:i",strtotime($transaction->created_at))}}</td>
													<td>{{ $booking->train_name }} - {{ $booking->train_number }}<br>
														<strong style="color: red;">{{ $booking->pnr }}</strong>
													</td>
													<td>{{ $booking->origin_station->city }} - {{ $booking->destination_station->city }}</td>
													<td>{{$transaction->buyer->name}}<br>{{$transaction->buyer->phone}}</td>
													{{-- <td style="text-align: right;">
														Smart Point : {{ (isset($booking->commission))?number_format($booking->commission->bv):'' }}<br>
														Smart Cash : {{ (isset($booking->commission))?number_format($booking->commission->member):'' }}
													</td> --}}
													@if(isset($booking->transaction_commission))
													<?php $smartpisrice+= $booking->paxpaid+$booking->admin-$booking->commission->member;?>
														<td style="text-align: right;">{{number_format($booking->paxpaid+$booking->admin-$booking->commission->member)}}</td>
													@else
													<?php $smartpisrice+= $booking->paxpaid+$booking->admin;?>
														<td style="text-align: right;">{{number_format($booking->paxpaid+$booking->admin)}}</td>
													@endif
													<td>
														@if($booking)
                                                            <?php
...
?>
													...
													<td><a href="{{route('trains.report_show',$transaction->id)}}" class="btn btn-md btn-primary"><span class="fa fa-info"></span></a> @if($booking->status=='booking')| <a href="{{ route('airlines.cancel',$transaction->id) }}" class="btn btn-md btn-danger"><span class="fa fa-close"></span></a>@endif</td>
													<td>
														@if($booking->status=='issued')
															<form action="{{ route('trains.receipts',$transaction->id) }}" method="GET" target="_blank"><input type="hidden" name="service_fee" class="service_fee" value="0">
								...
								<div class="col-md-4">
										<table class="text-right dataTable table table-striped table-bordered dt-responsive nowrap table-custom-blue">
											...
											<tr>
													...
													<td>IDR {{ number_format($smartpisrice) }}</td>
											</tr>
											{{-- <tr>
