...
@section('content')
		 <section class="section-pilih-seat">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
					 	<h3 class="blue-title-big">HISTORY RAILINK</h3>
					 	<div id="dataSearch" class="box-orange">
							<form action="{{route('railinks.reports')}}" method="GET">
								{{csrf_field()}}
							 <div class="row-form">
								 <div class="row">
									<div class="col-md-3">
											...
											@foreach($transaction->bookings()->where('status', 'LIKE', '%' . $reqStatus . '%')->get() as $booking)
												<tr>
													...
													@if(isset($booking->transaction_commission))
														<td style="text-align: right;">{{number_format($booking->paxpaid-$booking->commission->member)}}</td>
													@else
...
?>
													...
													<td><a href="{{route('railinks.report_show',$transaction->id)}}" class="btn btn-md btn-primary"><span class="fa fa-info"></span></a> @if($booking->status=='booking')| <a href="{{ route('airlines.cancel',$transaction->id) }}" class="btn btn-md btn-danger"><span class="fa fa-close"></span></a>@endif</td>
													<td>
														@if($booking->status=='issued')
															<form action="{{ route('railinks.receipts',$transaction->id) }}" method="GET" target="_blank"><input type="hidden" name="service_fee" class="service_fee" value="0">
